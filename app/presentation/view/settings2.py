...
bp_settings2 = Blueprint('settings2', __name__)

@bp_settings2.route('/settingsshow2', methods=['GET'])
@admin_required
@login_required
def show():
    cron_module_enable_settings = dl.settings.get_configuration_setting('cron-enable-modules')
    return render_template('settings2.html', data=[])


@bp_settings2.route('/setting', methods=['GET', 'UPDATE'])
@admin_required
@login_required
def setting():
    try:
...
@bp_settings2.route('/button', methods=['POST'])
@admin_required
@login_required
def button():
    try:
...
@bp_settings2.route('/setting/meta', methods=['GET'])
@login_required
def meta():
