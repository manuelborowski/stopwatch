...
al.socketio.subscribe_on_type("blabla-datatable-data", lambda type, data: datatable_get_data(config, data))

@bp_checkin.route('/checkin', methods=["POST", "UPDATE", "DELETE", "GET"])
@login_required
def checkin():
    ret = {}
...
@bp_checkin.route('/checkin/meta', methods=['GET'])
@login_required
def meta():
