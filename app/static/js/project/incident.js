...
class IncidentRepair {
    ...
    constructor(incident = null, history = "") {
    ...
    __state_select_set = () => {
        ...
        for (const state of [this.incident.incident_state].concat(next_state[this.incident.incident_type][this.incident.incident_state])) {
            incident_state_field.add(new Option(meta.label.incident_state[state], state, this.incident.incident_state === state));
        }
    }
    ...
    __password_field_visibility = (hidden = true) => {
        ...
        if (hidden) {
            ...
            password_field.value = this.stored_password;
        }
    }
    ...
    display = async () => {
        ...
        if (this.incident_update) {
            ...
            this.incident.m4s_category = meta.default.m4s_category;
            ...
            meta.option.laptop_serial = [{value: this.incident.laptop_serial, label: this.incident.laptop_name}];

            await form_populate(this.category, this.incident, meta);
            ...
            await form_populate(this.category, defaults, meta);
...
const __repair_form = async (incident = null, history = "") => {
    ...
    const repair = new IncidentRepair(incident, history);
